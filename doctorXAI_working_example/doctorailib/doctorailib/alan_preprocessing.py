...
def prepare_mimic(mimic_path, input_path, output_path, CSS=False):
    ...
    random.shuffle(pids)
    random.seed(seed)
    random.shuffle(visit_file)
    random.seed(seed)
    random.shuffle(label_file)
    random.seed(seed)
    random.shuffle(seqs)

    #
    splitting_threshold = len(pids) // 2
    pickle.dump(visit_file[:splitting_threshold], open(output_path + 'visit.train', 'wb'), protocol=-1)
    pickle.dump(label_file[:splitting_threshold], open(output_path + 'label.train', 'wb'), protocol=-1)
    pickle.dump(pids[:splitting_threshold], open(output_path + 'pid.train', 'wb'), protocol=-1)
    #
    pickle.dump(seqs[splitting_threshold:], open(output_path + 'icd9seq.valid', 'wb'), protocol=-1)
    pickle.dump(seqs[splitting_threshold:], open(output_path + 'icd9seq.test', 'wb'), protocol=-1)
    pickle.dump(visit_file[splitting_threshold:], open(output_path + 'visit.valid', 'wb'), protocol=-1)
    pickle.dump(visit_file[splitting_threshold:], open(output_path + 'visit.test', 'wb'), protocol=-1)
    pickle.dump(label_file[splitting_threshold:], open(output_path + 'label.valid', 'wb'), protocol=-1)
    pickle.dump(label_file[splitting_threshold:], open(output_path + 'label.test', 'wb'), protocol=-1)
    pickle.dump(pids[splitting_threshold:], open(output_path + 'pid.valid', 'wb'), protocol=-1)
    ...
    if CSS:
        ...
        print(
            "dr.train_doctorAI(seqFile=seqFile, inputDimSize=4880, labelFile=labelFile, numClass=272, outFile=outFile, max_epochs=50)")
    else:
        print(f'dr = doctorai.DoctorAI(ICD9_to_int_dict="{input_path}/ICD9_to_int_dict",verbose=True)')
        print(
            "dr.train_doctorAI(seqFile=seqFile, inputDimSize=4880, labelFile=labelFile, numClass=4880, outFile=outFile, max_epochs=50)")
